...
@click.option("--bk",              default=None,   help="Limit to one bookmaker slug")
@click.option("--sample",          is_flag=True,   help="Print one sample match per sport")
@click.option("--raw",             is_flag=True,   help="Print raw events for one game (mapper building)")
@click.option("--sports-tree",     is_flag=True,   help="Print sports/competitions tree")
@click.option("--save",            is_flag=True,   help="Save to Redis after harvest")
@click.option("--mapper",          is_flag=True,   help="Save best match per sport with ALL markets")
@click.option("--output-dir",      default="harvest_dumps")
@click.option("--headless",        default=True,   is_flag=False, type=bool,
              help="Run browser headless (--no-headless to watch)")
@click.option("--wait",            default=15,     help="Seconds to wait per competition page")
@click.option("--tabs",            default=5,      help="Concurrent tabs per BK (default 5)")
@click.option("--no-full-markets", is_flag=True,   help="Skip GetGameZip — faster, fewer markets")
def harvest_b2b_playwright(mode, sport, bk, sample, raw, sports_tree,
                            save, mapper, output_dir, headless, wait,
                            tabs, no_full_markets):
    """
    Harvest B2B bookmakers via Playwright — pure browser navigation.

    Navigates to each competition page so the browser fires the API requests
    naturally. x-hd tokens are handled automatically. All BKs run concurrently.

    \b
    Examples:
      flask harvest-b2b-playwright --sport soccer
      flask harvest-b2b-playwright --bk betwinner --sport soccer --no-headless
      flask harvest-b2b-playwright --mapper
      flask harvest-b2b-playwright --save
    """
    if not _check_pw():
        return
    ...
    # ── Mapper: best match per sport with all markets ─────────────────────────
    if mapper:
        os.makedirs(output_dir, exist_ok=True)
        ts      = datetime.now().strftime("%Y%m%d_%H%M%S")
        summary = {}
        ...
        summary_path = os.path.join(output_dir, f"mapper_summary_{ts}.json")
...
# ─── harvest-b2b-playwright-all ──────────────────────────────────────────────

@bp.cli.command("harvest-b2b-playwright-all")
@click.option("--output-dir", default="harvest_dumps")
@click.option("--sport",      default=None)
@click.option("--headless",   default=True,  is_flag=False, type=bool)
@click.option("--wait",       default=15)
@click.option("--debug",      is_flag=True)
def harvest_b2b_playwright_all(output_dir, sport, headless, wait, debug):
    """
    ...
    Examples:
      ...
      flask harvest-b2b-playwright-all --no-headless
    """
    if not _check_pw():
        return
...
def b2b_pw_setup(bk):
    """
    Open browser to log in — saves session cookies for future harvests.
    ...
    with sync_playwright() as pw:
        ...
        # Save storage state (cookies + localStorage) for future headless runs
        try:
            state = ctx.storage_state()
            with open(storage_f, "w") as f:
                json.dump(state, f)
            click.echo(f"  💾 Storage state saved → {storage_f}")
        except Exception as e:
            click.echo(f"  ⚠️  Could not save storage state: {e}")
...
@click.option("--headless", default=True, is_flag=False, type=bool)
@click.option("--wait",     default=15)
@click.option("--tabs",     default=5,   help="Concurrent tabs per BK")
def b2b_pw_test(bk, sport, mode, headless, wait, tabs):
    """
    ...
    Examples:
      ...
      flask b2b-pw-test --sport ice-hockey
      flask b2b-pw-test --bk betwinner --no-headless
      flask b2b-pw-test --bk paripesa --sport esoccer
    """
    if not _check_pw():
        return
    ...
    click.echo(f"   Navigating to each competition page naturally…\n")
...
@bp.cli.command("test-sp-harvester")
...
@bp.cli.command("show-normalized")
...
@bp.cli.command("fast-sp-harvest")
...
@bp.cli.command("fast-od-harvest")
@click.option("--days", default=7, help="Days ahead")
@click.option("--batch-size", default=500, help="Matches per batch")
@click.option("--max-workers", default=8, help="Max parallel batches across sports")
@click.option("--full-markets/--no-full-markets", default=True, help="Fetch full markets (slower)")
...
@bp.cli.command("fetch-od-complete")
...
@bp.cli.command("debug-od-esoccer")
...
@bp.cli.command("fetch-bt-complete")
...
@bp.cli.command("fetch-sp-complete")
...
# -----------------------------------------------------------------------------
@bp.cli.command("harvest-bookmaker")
...
# -----------------------------------------------------------------------------
@bp.cli.command("harvest-unified")
@click.option("--days", default=7, help="Days ahead")
...
def register_harvest_view(app=None):
    @bp.route("/harvest/view/<filename>")
...
TEST_ACCOUNTS = [
    {"email": "basic@oddskenya.com",      "display_name": "Basic Tester",      "password": "Test1234!", "tier": "basic"},
    {"email": "pro@oddskenya.com",        "display_name": "Pro Tester",        "password": "Test1234!", "tier": "pro"},
    {"email": "premium@oddskenya.com",    "display_name": "Premium Tester",    "password": "Test1234!", "tier": "premium"},
    {"email": "enterprise@oddskenya.com", "display_name": "Enterprise Tester", "password": "Test1234!", "tier": "premium"},
    {"email": "admin@oddskenya.com",      "display_name": "Admin Tester",      "password": "Test1234!", "tier": "admin"},
]

_TIER_COLOR = {"basic": "\033[94m", "pro": "\033[92m", "premium": "\033[93m", "admin": "\033[95m"}
_RESET = "\033[0m"; _BOLD = "\033[1m"; _RED = "\033[91m"; _DIM = "\033[2m"

def _color(tier, text):
    return f"{_TIER_COLOR.get(tier, '')}{text}{_RESET}"

def _upsert_account(cfg, verbose=True):
    from app.extensions import db
    ...
    if user:
        # Refresh password + verification — never touch Customer.tier (it's on Subscription)
        user.is_active   = True
        user.is_verified = True
        user.set_password(cfg["password"])
        action = "updated"
    else:
        # Build Customer with only the columns that actually exist on the model
        kwargs = {"email": email, "is_active": True, "is_verified": True}
        # display_name is optional — skip silently if the column doesn't exist
        try:
            user = Customer(display_name=cfg["display_name"], **kwargs)
        except TypeError:
            user = Customer(**kwargs)
        user.set_password(cfg["password"])
        db.session.add(user)
        db.session.flush()   # need user.id before FK writes
        action = "created"
    ...
    if verbose:
        tag   = _color(tier, f"[{tier.upper():>9}]")
...
@bp.cli.command("seed-accounts")
@click.option("--quiet", "-q", is_flag=True)
def seed_accounts(quiet):
    """Create/update all 5 tier test accounts (no email verification needed)."""
    click.echo(f"\n{_BOLD}Seeding test accounts…{_RESET}\n")
    for cfg in TEST_ACCOUNTS:
        _upsert_account(cfg, verbose=not quiet)
    click.echo(f"\n{_BOLD}Done.{_RESET}  Password for all: {_DIM}Test1234!{_RESET}\n")
    click.echo("  flask get-token pro@oddskenya.com")
    click.echo("  flask list-accounts\n")

@bp.cli.command("get-token")
@click.argument("email")
@click.option("--curl", is_flag=True, help="Also print curl examples")
@click.option("--base-url", default="http://localhost:5000", show_default=True)
def get_token(email, curl, base_url):
    """Print a fresh 24-hour JWT for a test account."""
    from app.models.customer import Customer
    user = Customer.query.filter_by(email=email.lower().strip()).first()
    if not user:
        click.echo(f"\n{_RED}✗ Not found: {email}  →  run flask seed-accounts first{_RESET}\n")
        sys.exit(1)
    ...
    if curl:
        ...
        click.echo(f"  curl -H 'Authorization: Bearer {token}' '{base_url}/odds/snapshot/upcoming/soccer'\n")
        click.echo(f"  curl -N '{base_url}/odds/stream/upcoming/soccer?token={token}'\n")
        click.echo(f"  curl -H 'Authorization: Bearer {token}' '{base_url}/api/monitor/stats'\n")

@bp.cli.command("list-accounts")
def list_accounts():
    """Show all test accounts with tier and verification status."""
    from app.models.customer import Customer
    emails = {c["email"] for c in TEST_ACCOUNTS}
    users  = Customer.query.filter(Customer.email.in_(emails)).all()
    if not users:
        click.echo(f"\n{_DIM}No test accounts found — run flask seed-accounts{_RESET}\n")
        return
    click.echo(f"\n  {'EMAIL':<35} {'TIER':<10} {'VERIFIED':<10} ACTIVE")
    click.echo("  " + "─" * 62)
    for u in sorted(users, key=lambda x: x.email):
        tier = u.tier or "basic"
        click.echo(f"  {_color(tier, u.email):<44} {_color(tier, tier):<19} {'✓' if u.is_verified else '✗':<10} {'✓' if u.is_active else '✗'}")
    click.echo()

@bp.cli.command("delete-account")
@click.argument("email")
@click.confirmation_option(prompt="Permanently delete this account?")
def delete_account(email):
    """Permanently remove a test account."""
    from app.extensions import db
    from app.models.customer import Customer
    user = Customer.query.filter_by(email=email.lower().strip()).first()
    if not user:
...
@bp.cli.command("test-login")
@click.argument("email")
@click.option("--password", default="Test1234!", show_default=True)
def cli_login(email, password):
    """Prints just the raw token — capture with TOKEN=$(flask test-login pro@oddskenya.com)."""
    from app.models.customer import Customer
    user = Customer.query.filter_by(email=email.lower().strip(), is_active=True).first()
    if not user or not user.check_password(password):
        click.echo(f"{_RED}✗ Bad credentials{_RESET}", err=True); sys.exit(1)
    click.echo(_issue_token_for(user.id))

# ─── Registration helper ──────────────────────────────────────────────────────


@bp.cli.command("push-bt-to-redis")
@click.option("--sport", default="soccer")
...
@bp.cli.command("push-od-to-redis")
@click.option("--sport", default="soccer")
...
@bp.cli.command("list-bookmakers")
def list_bookmakers():
...
@bp.cli.command("market-failures")
