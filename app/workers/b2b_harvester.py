...
def _build_curl_command(url: str, referer: str) -> str:
    """Return the exact curl command string that would reproduce this request."""
    header_args = " ".join(
        f'-H "{k}: {v}"' for k, v in _BROWSER_HEADERS.items()
    )
    return f'curl -s -m 20 {header_args} -H "referer: {referer}" "{url}"'
...
def _curl_get(url: str, referer: str) -> dict | None:
    ...
    try:
        ...
        if res.returncode != 0 or not res.stdout.strip():
            logger.debug("[curl] non-zero exit or empty body: code=%d", res.returncode)
            return None
        return json.loads(res.stdout)
    except json.JSONDecodeError as exc:
        logger.debug("[curl] JSON decode error: %s", exc)
        return None
    except Exception as exc:
        logger.debug("[curl] exception: %s", exc)
        return None
