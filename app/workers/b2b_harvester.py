...
BetB2B family harvester — covers all 7 Kenya bookmakers:
  ...
  ────────────┼─────────────────────┼───────────┼─────┼──────────
  1xBet       | 1xbet.co.ke         | 61        | 657 | LiveFeed
...
# Full set of headers that a real browser sends (extracted from the provided
# working curl sample).  The dynamic "x-hd" header is omitted because it is
# generated by JavaScript and changes per session.
_BROWSER_HEADERS = {
...
def _curl_get(url: str, referer: str, debug: bool = False) -> dict | None:
    """Use system curl with a full set of browser headers."""
    cmd = [
        "curl", "-s", "-m", "15",
        "-H", f"referer: {referer}",
        *[item for key, val in _BROWSER_HEADERS.items() for item in ("-H", f"{key}: {val}")],
        url,
    ]
...
def _log_curl(output_dir: str, bk_slug: str, sport_id: int, mode: str,
              url: str, referer: str, success: bool):
    """Append the exact curl command (ready to copy/paste) to the shared log file."""
    if not output_dir:
        return
    ...
    header_args = " ".join(
        f'-H "{key}: {val}"' for key, val in _BROWSER_HEADERS.items()
    )
    cmd_line = (
        f'curl -s -m 15 {header_args} '
        f'-H "referer: {referer}" '
        f'"{url}"'
    )
    ...
    line = f"[{timestamp}] {bk_slug:>10} | sport={sport_id:<3} | {mode:<9} | {status:<4} | {cmd_line}\n"
...
def _build_livefeed_url(bk: dict) -> str:
    domain = bk["domain"]
    pid    = bk["partner_id"]
    gr     = bk.get("gr")
...
def _fetch_b2b_raw(bk: dict, sport_id: int, mode: str = "upcoming",
    ...
    if output_dir:
        _log_curl(output_dir, slug, sport_id, mode, url, referer, success)
    ...
    all_games = data.get("Value") or data.get("value") or []

    # Filter by sport_id for LiveFeed
    if bk["feed"] == "LiveFeed":
        raw_sport_games = [g for g in all_games if isinstance(g, dict) and g.get("SI") == sport_id]
    else:
...
def _parse_events(raw_events: list[dict], additional_events: list[dict]) -> dict[str, dict[str, float]]:
    ...
    def _process_event(ev: dict):
        ...
        slug = _B2B_GROUP_TO_SLUG.get(gid, f"unknown_{gid}")
...
def fetch_single_bk(
    ...
    raw_games = _fetch_b2b_raw(bk, sport_id, mode, debug=debug, output_dir=output_dir)
...
def fetch_all_b2b_sport(
    ...
    with ThreadPoolExecutor(max_workers=min(max_workers, len(bks)), thread_name_prefix="b2b") as pool:
        futures = {
            pool.submit(fetch_single_bk, bk, sport_slug, mode, 1, 200, debug, output_dir): bk
            for bk in bks
        }
