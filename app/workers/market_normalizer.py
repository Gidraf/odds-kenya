"""
app/workers/market_normalizer.py
=================================
...
# ──────────────────────────────────────────────────────────────────────────────
# CACHED MARKET SLUG NORMALIZER
# ──────────────────────────────────────────────────────────────────────────────
@lru_cache(maxsize=4096)
def normalize_market_slug(raw_slug: str, h_clean: str, a_clean: str) -> str:
    """Canonical slug transform. Cached — same inputs always produce same output."""
    m = raw_slug.lower()
...
# ──────────────────────────────────────────────────────────────────────────────
# BEST-ODDS MERGE  (mutates `best` in-place)
# ──────────────────────────────────────────────────────────────────────────────
def merge_best(best: dict, markets_by_bk: dict) -> None:
        ...
        for mkt_slug, outcomes in mkts.items():
            ...
            for out_key, price_dict in outcomes.items():
                ...
                if existing is None or p > existing["odd"]:
                    bk_mkt[out_key] = {"odd": p, "bk": bk_slug}


# ──────────────────────────────────────────────────────────────────────────────
# UNIVERSAL MATCH NORMALIZER
# ──────────────────────────────────────────────────────────────────────────────
def unify_match_payload(
    raw_match: dict,
    count:     int,
    mode:      str,
    bk_slug:   str,
    ...
    """Convert any bookmaker raw dict → canonical unified match payload."""
    if not raw_match:
        return {}
    ...
    for mkt_slug, outcomes in standardized_markets.items():
        ...
        m = normalize_market_slug(str(mkt_slug), h_clean, a_clean)
        bk_markets[m] = {}
        best_mock[m]  = {}
        ...
        for ok_raw, p in outcomes.items():
            ...
            if "double_chance" in m:
                ...
                if prefix and suffix: ok = f"{prefix} {suffix}"
                    ...
                    ok = (ok_lower.replace(" & "," ").replace("and","").replace("yes","Yes").replace("no","No")
