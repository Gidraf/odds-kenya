...
def get_universal_market_slug(sid: str, n: str, specs: dict) -> str:
    ...
    # Winner Markets (Moneyline, 1X2, Match Winner)
    ...
    # Handicaps (Asian, European, Game, Point)
    if "handicap" in n:
...
def _normalise_match(raw: dict, *, source: str = "upcoming") -> dict | None:
    try:
        ...
        start_time    = str(raw.get("start_time") or "").replace(" ", "T")
        ...
        return {
            ...
            "source":            "betika",
            "is_live":           source == "live",
            "match_time":        str(raw.get("match_time") or ""),
            "event_status":      str(raw.get("event_status") or ""),
            "current_score":     str(raw.get("current_score") or ""),
            "markets":           markets,
...
# ══════════════════════════════════════════════════════════════════════════════
# BACKGROUND LIVE POLLER & EXPORTS
# ══════════════════════════════════════════════════════════════════════════════
...
__all__ = ["fetch_upcoming_matches", "fetch_live_matches", "fetch_upcoming_stream", "fetch_live_stream", "get_full_markets"]