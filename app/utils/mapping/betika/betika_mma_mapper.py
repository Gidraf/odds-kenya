...
class BetikaMMAMapper:
    ...
    MARKET_MAP = {
        ...
        "186": "mma_winner",  # Winner (2-Way)
        "1":   "mma_1x2",     # Fallback if they ever use a 3-Way (1X2)
    ...
    def format_line(raw_line: str) -> str:
        """Formats string specifiers into canonical lines (e.g., '2.5' -> '2_5')."""
        if not raw_line:
            return ""
    ...
    def get_market_slug(cls, sub_type_id: str, parsed_specifiers: dict, fallback_name: str = "") -> str:
        ...
        # Fallback if ID is unknown (prepends "mma_" to keep it organized)
        if not base_slug:
    ...
    def normalize_outcome(display: str) -> str:
        ...
        # 1. Strip out Line annotations in parenthesis e.g., "OVER (2.5)" -> "OVER"
