...
Key changes vs previous:
  • Sibling merge REMOVED from _load_db_matches (list endpoint).
    It caused full-table-scan OR queries on every page load → timeouts.
    With correctly keyed persistence (bt_parent_id / od_parent_id as
    betradar_id) there are no duplicate rows to merge anymore.
  • Sibling merge KEPT on get_match (single row, runs once, acceptable).
  • _load_db_matches uses only indexed queries:
      - UnifiedMatch filtered by sport_name (indexed)
      - BookmakerMatchOdds fetched with match_id IN (page ids)
      - No func.lower() on large tables
  • _sport_filter uses indexed equality checks, not ilike.
  • per_page capped at 100 (was already there, kept).
  • Fallback to Redis cache on any DB error.
"""
...
# ── Sport aliasing ────────────────────────────────────────────────────────────
# Values in this dict must match EXACTLY what is stored in unified_matches.sport_name
# so the DB query can use an indexed equality check (not ilike / lower()).
...
def _sport_filter(q, sport_slug: str):
    """
    Filter query by sport using indexed equality checks.
    Uses OR across canonical DB names — no func.lower(), no ilike.
    """
...
def _flatten_db_markets(raw_markets: dict) -> dict:
    """
    Flatten nested specifier-keyed market dict into {market: {outcome: val}}.

    BookmakerMatchOdds.markets_json structure:
      { "1x2": { "": { "1": {...}, "X": {...}, "2": {...} } } }
    or already flat:
      { "1x2": { "1": {...}, "X": {...}, "2": {...} } }
    """
    ...
    for mkt_slug, spec_dict in (raw_markets or {}).items():
        ...
        for spec_val, inner in spec_dict.items():
            if isinstance(inner, dict):
                # inner is {outcome: value}
                for out_key, out_val in inner.items():
                    outcomes[out_key] = out_val
            else:
                # already {outcome: value} (no specifier layer)
...
# ══════════════════════════════════════════════════════════════════════════════
# MAIN DB LOADER  (list endpoints — no sibling merge)
# ══════════════════════════════════════════════════════════════════════════════
...
def _load_db_matches(
    ...
    # ── Build base query (all indexed filters) ────────────────────────────────
    ...
    # ilike is acceptable on filtered subsets (competition/team are secondary filters)
    if comp_filter:
    ...
    # ── Bookmaker odds — single IN query, no sibling merge ───────────────────
    bmo_rows = BookmakerMatchOdds.query.filter(
        BookmakerMatchOdds.match_id.in_(match_ids)
    ).all()

    # ── Resolve bookmaker names in one query ──────────────────────────────────
    ...
    # ── Bookmaker match links (for deep-link URLs) ────────────────────────────
    ...
    for um in um_list:
        ...
        for bmo in bmos:
            ...
            if slug in bookmakers:
                # Same bookmaker has two BMO rows — merge markets
        ...
        # ── Inline arb detection (cross-bookmaker) ────────────────────────────
        ...
        result.append({
            ...
            "join_key": (
                f"br_{um.parent_match_id}" if um.parent_match_id else f"db_{um.id}"
            ),
...
def _read_cache_sources(mode: str, sport_slug: str) -> list[dict]:
    ...
    def _fetch_prefix(prefix: str, slug: str) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=min(16, len(work) or 1)) as pool:
        futures = {pool.submit(_fetch_prefix, p, s): (p, s) for p, s in work}
    ...
    for s in slugs_to_try:
        extra_keys += (
            cache_keys(f"odds:{mode}:{s}:*") +
            cache_keys(f"odds:{mode}:{s}")
        )
    if extra_keys:
        with ThreadPoolExecutor(max_workers=min(8, len(extra_keys))) as pool:
            for fut in as_completed(
                {pool.submit(_fetch_key, k): k for k in extra_keys}
            ):
                try:
                    raw.extend(fut.result())
                except Exception:
...
def _normalise_cache_match(m: dict) -> dict:
    ...
    if src and raw_mkts and isinstance(raw_mkts, dict):
        first_val = next(iter(raw_mkts.values()), None)
        if isinstance(first_val, dict):
            inner = next(iter(first_val.values()), None)
            if isinstance(inner, (int, float)):
                # flat {outcome: price} keyed by market
                markets_by_bk[src] = raw_mkts
            else:
                # already {bk_slug: {market: outcomes}}
    ...
    return {
        ...
        "join_key": (
            f"br_{m['betradar_id']}" if m.get("betradar_id")
...
def _apply_filters(matches: list[dict], args) -> list[dict]:
    ...
    result = []
    for m in matches:
        if bookmaker and not any(bookmaker in k for k in (m.get("bookmakers") or {})):
            continue
        if market and not any(
            market in k for k in (m.get("best_odds") or m.get("best") or {})
        ):
            continue
        if team and (
            team not in m.get("home_team", "").lower() and
...
def _build_envelope(
    matches, sport, mode, tier, page, per_page,
    ...
    env = {
        "ok":           True,
        "sport":        _normalise_sport_slug(sport),
        "mode":         mode,
        "tier":         tier,
        "total":        _total,
        "page":         page,
...
def list_sports():
    ...
    for prefix in _CACHE_PREFIXES:
        for k in cache_keys(f"{prefix}:upcoming:*"):
            parts = k.split(":")
            if len(parts) >= 3:
                sp = _normalise_sport_slug(parts[2].replace("_", " "))
                sports.setdefault(sp, 0)
    return _signed_response({
        "ok": True, "sports": sorted(sports.keys()), "counts": sports,
    })
...
def list_bookmakers():
    ...
    bk_counts = dict(
        ...
        .with_entities(
            ...
            func.count(BookmakerMatchOdds.match_id),
        )
...
def get_upcoming(sport_slug: str):
    ...
    return _signed_response(
        _build_envelope(
            matches, sport_slug, "upcoming", tier,
...
def _get_finished_by_date(date_str: str):
    ...
    return _signed_response(
        _build_envelope(
            matches, date_str, "finished", tier,
            page, per_page, truncated, latency,
...
# ══════════════════════════════════════════════════════════════════════════════
# SINGLE MATCH DETAIL  (sibling merge kept — runs once, acceptable cost)
# ══════════════════════════════════════════════════════════════════════════════
...
def get_match(parent_match_id: str):
    ...
    bmos: list = list(BookmakerMatchOdds.query.filter_by(match_id=um.id).all())

    # Sibling merge — find duplicate rows for same real-world event
    # (only runs once per request so the extra queries are acceptable)
    try:
...
class _SiblingBMO:
    """Wraps a BookmakerMatchOdds row, overriding match_id to the primary row."""
...
def search_matches():
    ...
    if mode == "live":
        qs = qs.filter(UnifiedMatch.status.in_(["IN_PLAY", "live"]))
    elif mode == "finished":
    ...
    bk_counts = dict(
        db.session.query(
            ...
            sqlfunc.count(BookmakerMatchOdds.bookmaker_id),
        )
