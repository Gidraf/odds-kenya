...
# Bookmaker name → canonical 2-letter slug
...
def _load_db_matches(
    ...
    """
    Query UnifiedMatch + BookmakerMatchOdds from PostgreSQL.
    Returns (matches_list, total_count, pages).

    Each match dict contains:
      match_id, parent_match_id, betradar_id, home_team, away_team,
      competition, sport, start_time, status,
      bookmakers: {bk_slug: {bookmaker, markets, market_count, link}},
      markets_by_bk: {bk_slug: {slug: {out: odd}}},
      best: {slug: {out: {odd, bk}}},
      has_arb, arb_markets, best_arb_pct
    """
    ...
    if mode == "upcoming":
        ...
        q = q.filter(
            or_(
                UnifiedMatch.start_time >= now - timedelta(hours=1),
                UnifiedMatch.start_time.is_(None),
            ),
            or_(
                UnifiedMatch.status.in_(["PRE_MATCH", "upcoming", "PREMATCH"]),
    ...
    q = q.order_by(sort_col.asc() if sort == "start_time" else sort_col.asc())
    ...
    # Load all bookmaker odds in one query
    ...
    for um in um_list:
        ...
        # Build best_odds (legacy shape: {mkt: {out: {odd, bookie}}})
        best_odds: dict[str, dict] = {
            mkt: {out: {"odd": v["odd"], "bookie": v["bk"]}
                  for out, v in outcomes.items()}
            for mkt, outcomes in best.items()
        }

        # Detect arb from best map
        ...
        result.append({
            ...
            # ── Odds (multiple shapes for compatibility) ───────────────────────
            "bookmakers":      bookmakers,         # {slug: {markets, ...}}
            "markets_by_bk":   markets_by_bk,      # {slug: {mkt: {out: odd}}}
            "markets":         markets_by_bk,      # alias for component
            "best":            best,               # {mkt: {out: {odd, bk}}}
            "best_odds":       best_odds,          # legacy shape
            "aggregated":      um.markets_json or {},
            # ── Opportunities ─────────────────────────────────────────────────
            "has_arb":         has_arb_flag,
            "arb_markets":     arb_markets,
            "arbs":            arb_markets,        # alias for component
...
# =============================================================================
# ── LEGACY: Redis cache reader (fallback / enrichment) ────────────────────────
# =============================================================================
...
def _normalise_cache_match(m: dict) -> dict:
    """Normalise a Redis-cache match into the same shape as _load_db_matches."""
    home = str(m.get("home_team") or m.get("home_team_name") or "")
    away = str(m.get("away_team") or m.get("away_team_name") or "")

    # Build markets_by_bk from various cache shapes
    markets_by_bk: dict[str, dict] = {}
    ...
    if src and raw_mkts:
        ...
        # Already in bk-keyed shape?
        first_val = next(iter(raw_mkts.values()), None)
        if isinstance(first_val, dict):
            inner = next(iter(first_val.values()), None)
            if isinstance(inner, (int, float)):
                # {slug: {out: odd}} — single-bk shape
                slug = _bk_slug(src or "sp")
                markets_by_bk[slug] = raw_mkts
            else:
                # {bk: {slug: {out: odd}}} — already nested
...
def _apply_filters(matches: list[dict], args) -> list[dict]:
    ...
    for m in matches:
        if bookmaker and not any(bookmaker in k for k in (m.get("bookmakers") or {})):
            continue
        if market and not any(market in k for k in (m.get("best_odds") or m.get("best") or {})):
            continue
        if team:
...
def _sort_matches(matches: list[dict], sort: str) -> list[dict]:
    def _dt(m):
        try:
            return datetime.fromisoformat(str(m.get("start_time","")).replace("Z","+00:00"))
        except Exception:
            return datetime.max.replace(tzinfo=timezone.utc)
    if sort == "arb":
        return sorted(matches, key=lambda m: -(m.get("best_arb_pct") or 0))
    if sort == "market_count":
        return sorted(matches, key=lambda m: -(m.get("market_count") or 0), reverse=False)
    if sort == "bk_count":
...
def _build_envelope(
    matches, sport, mode, tier,
    page, per_page, truncated, latency_ms,
    total=None, pages=None, extra=None,
) -> dict:
...
def list_sports():
    ...
    # Also scan Redis for sports not yet in DB
...
def list_bookmakers():
    ...
    bk_counts = dict(
        ...
        .with_entities(BookmakerMatchOdds.bookmaker_id,
...
def get_match(parent_match_id: str):
    ...
    # Odds history (last 50)
    history = (BookmakerOddsHistory.query
...
def search_matches():
    ...
    now = datetime.now(timezone.utc)
    if mode == "upcoming":
        qs = qs.filter(UnifiedMatch.start_time >= now - timedelta(hours=1))
    elif mode == "live":
    ...
    # For search results we do a lightweight response — full markets on demand
    from app.models.odds_model import BookmakerMatchOdds
...
def harvest_status():
    ...
    try:
        ...
        bk_cov   = dict(
            db.session.query(BookmakerMatchOdds.bookmaker_id,
                             func.count(BookmakerMatchOdds.match_id))
            .group_by(BookmakerMatchOdds.bookmaker_id).all()
        )
