...
def _flatten_db_markets(raw_markets: dict) -> dict:
    ...
    for mkt_slug, spec_dict in (raw_markets or {}).items():
        ...
        if isinstance(spec_dict, dict):
            for _spec_val, outcomes in spec_dict.items():
...
def _load_db_matches(
    ...
    for um in um_list:
        ...
        all_slugs = sorted({slug for bk_mkts in markets_by_bk.values() for slug in bk_mkts})
        ...
        for mkt, outcomes in bk_mkts.items():
            ...
            for out, odd in (outcomes or {}).items():
                ...
                if fv > 1.0 and (out not in best[mkt] or fv > best[mkt][out]["odd"]):
                    best[mkt][out] = {"odd": fv, "bk": sl}
    slugs     = sorted({s for bk in markets_by_bk.values() for s in bk})
...
def _apply_filters(matches: list[dict], args) -> list[dict]:
    ...
    for m in matches:
        ...
        if market and not any(market in k for k in (m.get("best_odds") or m.get("best") or {})):
            continue
        if (team and team not in m.get("home_team", "").lower()
                and team not in m.get("away_team", "").lower()):
            continue
...
def _build_envelope(matches, sport, mode, tier, page, per_page, truncated,
    ...
    sport_out = _normalise_sport_slug(sport)
    env = {
        "ok": True, "sport": sport_out, "mode": mode, "tier": tier,
...
def get_match(parent_match_id: str):
    ...
    history = (
        BookmakerOddsHistory.query.filter_by(match_id=um.id)
