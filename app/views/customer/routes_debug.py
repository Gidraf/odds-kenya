    ...
    def _gen():
        ...
        try:
            ...
            for sp_match in stream:
                ...
                for mkt, outs in sp_match.get("markets", {}).items():
                ...
                ui_match = {
                    "match_id": int(sp_match.get("sp_game_id") or count),
                    "join_key": f"br_{sp_match['betradar_id']}" if sp_match.get("betradar_id") else f"sp_{sp_match.get('sp_game_id')}",
                    ...
                    "is_live": mode == "live", "bk_count": 1, "market_count": sp_match.get("market_count", 0),
                    "market_slugs": list(sp_match.get("markets", {}).keys()),
                    "bookmakers": {"sp": {"bookmaker": "SPORTPESA", "slug": "sp", "markets": bk_markets, "market_count": sp_match.get("market_count", 0)}},
    ...
    def _gen():
        ...
        try:
            ...
            for bt_match in stream:
                count += 1
                best_mock = {}; bk_markets = {}
                for mkt, outs in bt_match.get("markets", {}).items():
                    best_mock[mkt] = {}; bk_markets[mkt] = {}
                    for out_key, price in outs.items():
                        best_mock[mkt][out_key] = {"odd": price, "bk": "bt"}
                        bk_markets[mkt][out_key] = {"price": price}

                ui_match = {
                    "match_id": int(bt_match.get("bt_game_id") or bt_match.get("id") or count),
                    "join_key": f"br_{bt_match['betradar_id']}" if bt_match.get("betradar_id") else f"bt_{bt_match.get('bt_game_id')}",
                    "parent_match_id": bt_match.get("betradar_id"), "home_team": bt_match["home_team"], "away_team": bt_match["away_team"],
                    "competition": bt_match["competition"], "sport": _normalise_sport_slug(bt_match["sport"]),
                    "start_time": bt_match["start_time"], "status": "IN_PLAY" if mode == "live" else "PRE_MATCH",
                    "is_live": mode == "live", "bk_count": 1, "market_count": bt_match.get("market_count", 0),
                    "market_slugs": list(bt_match.get("markets", {}).keys()),
                    "bookmakers": {"bt": {"bookmaker": "BETIKA", "slug": "bt", "markets": bk_markets, "market_count": bt_match.get("market_count", 0)}},
                    "markets_by_bk": {"bt": bk_markets}, "best": best_mock, "best_odds": best_mock,
                    "has_arb": False, "has_ev": False, "has_sharp": False
                }
                yield _sse("batch", {"matches": [ui_match], "batch": count, "of": "unknown", "offset": count - 1})
                yield _keepalive()
