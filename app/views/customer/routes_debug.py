...
def debug_stream_sportpesa(mode: str, sport_slug: str):
    ...
    def _gen():
        ...
        yield _sse("meta", {
            "source": "sportpesa_direct_harvester", 
            "sport": _normalise_sport_slug(sport_slug), 
            "mode": mode, "fetch_full_markets": fetch_full,
            "now": _now_utc().isoformat(), "total": max_m if mode == "upcoming" else 100
        })
        
        try:
            if mode == "live": stream = fetch_live_stream(sport_slug, fetch_full_markets=fetch_full, debug_ou=debug_ou)
            else: stream = fetch_upcoming_stream(sport_slug, max_matches=max_m, fetch_full_markets=fetch_full, debug_ou=debug_ou)
            
            count = 0
            for sp_match in stream:
                count += 1
                mkt_slugs = list(sp_match["markets"].keys())
                best_mock = {}
                bk_markets = {}
                ...
                ui_match = {
                    ...
                    "parent_match_id": sp_match.get("betradar_id"),
                    "home_team": sp_match["home_team"], "away_team": sp_match["away_team"],
