...
def _effective_status(db_status: str | None, start_time: datetime | None) -> str:
    if db_status in config._TERMINAL_STATUSES:
        return db_status
...
def _is_upcoming_safe(db_status, start_time) -> bool:
    if (db_status or "").upper() in config._EXCLUDE_FROM_UPCOMING:
        return False
    if not start_time:
        return False
    st = start_time if start_time.tzinfo else start_time.replace(tzinfo=timezone.utc)
...
def _bk_slug(name: str) -> str:
    return config._BK_SLUG.get(name.lower(), name.lower()[:4])
...
def _apply_tier_limits(matches, user):
    import app.views.odds_feed.config as cfg
    if cfg.FREE_ACCESS: return matches, False
    limits = (user.limits if user else None) or {"max_matches": cfg.FREE_MATCH_LIMIT}
    max_m  = limits.get("max_matches") or cfg.FREE_MATCH_LIMIT
...
def _get_country_weight(sport_slug: str, category_name: str) -> int:
    ...
    weights = config._POPULARITY_WEIGHTS.get(sport_slug, {})
