...
def get_navigation_tree():
    ...
    try:
        rows = (
            db.session.query(UnifiedMatch.sport_name, UnifiedMatch.competition_name, func.count(UnifiedMatch.id))
            .filter(UnifiedMatch.start_time > now)
            .filter(UnifiedMatch.status.notin_(list(config._TERMINAL_STATUSES)))
        ...
        for sport, comp, count in rows:
            ...
            if category not in nav_tree[sport_slug]["categories"]:
                nav_tree[sport_slug]["categories"][category] = {"name": category, "weight": _get_country_weight(sport_slug, category), "count": 0, "competitions": {}}
